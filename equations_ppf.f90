    ...
    real(dl), intent(out) :: new_trans
    real(dl) :: calA, omegade, nu, a, kappa_t, fac, num1, num2, num3, logterm, tanterm
    !real :: tanterm,logterm,num1,num2,num3,fac
    ...
    num2 = (1/(6*nu**(1./3.)))*log(logterm)

    tanterm = (sqrt(3.0d0)/3)*(2.*a-nu**(1./3.))/(nu**(1./3.))

    num3 = sqrt(3.0d0)/(3*nu**(1./3.))*atan(tanterm)

    calA = fac*(num1+num2+num3)

    call BJL(0,kappa_t*calA,new_trans)
...
if (debugtrans) then
             ...
             z_test = 0
             do i=1,1001
                cappa = 0.001 + (i-1)*(1-0.001)/1000
                if (i.eq.1) then
                   test=1
                   call get_udm_trans(CP,cappa,z_test,new_trans,test)
                else
                   call get_udm_trans(CP,cappa,z_test,new_trans)
                end if
...
!
             ...
             cappa = 0.1
             do i=1,1001
                z_test = 0.01 + (i-1)*(1-0.01)/1000
                if (i.eq.1) then
                   test=1
                   call get_udm_trans(CP,cappa,-1+1/z_test,new_trans,test)
                else
                   call get_udm_trans(CP,cappa,-1+1/z_test,new_trans)
                end if
                write(42,*) z_test,new_trans
             end do
             close(42)
             stop
end if
